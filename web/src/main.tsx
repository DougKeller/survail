...
function decklistText(deck: Deck): string {
  const zoneOrder: readonly CardZone[] = [
    "commander", "mainboard", "sideboard", "companion", "considering",
  ];
  return zoneOrder
    .map((zone) => {
...
function bulkEditChanges(deck: Deck, preview: MoxfieldImportPreview): DeckOperationChangeInput[] {
  const identity = (printingId: string, finish: CardFinish, zone: CardZone): string =>
    `${printingId}:${finish}:${zone}`;
  const existing = new Map(
    deck.cardsets.map((card) => [identity(card.printing_id, card.finish, card.zone), card]),
  );
  const desired = new Map(
    preview.cardsets.map((card) => [identity(card.printing_id, card.finish, card.zone), card]),
  );
...
function isAgentActivityEvent(
  event: AgentUiEvent,
): event is Extract<AgentUiEvent, { type: "run_started" | "status" | "model_started" | "heartbeat" | "tool_started" | "tool_completed" }> {
...
function GeneratedDescription({ description, cards }: { description: string; cards: CardSet[] }) {
  ...
  return <div className="generated-description" aria-live="polite">
    {lines.map((line, index) => {
      if (line.startsWith("# ")) {
        return <h3 key={String(index)}><InlineCardText cards={cards} text={line.slice(2)} /></h3>;
      }
      if (line.startsWith("- ")) {
        return <div className="gameplan-item" key={String(index)}><MaterialIcon name="arrow_right" /><p><InlineCardText cards={cards} text={line.slice(2)} /></p></div>;
      }
      if (line.trim() === "") return null;
      return <p key={String(index)}><InlineCardText cards={cards} text={line} /></p>;
    })}
  </div>;
}
...
function storedDeckDisplayPreferences(): DeckDisplayPreferences {
  ...
  try {
    ...
    if (
      ...
      || (sortBy !== "alphabetical" && sortBy !== "mana-value" && sortBy !== "price" && sortBy !== "score")
    ) {
...
function groupedCards(
  ...
  return [...groups.entries()].map(([label, groupCards]) => ({
    ...
    cards: [...groupCards].sort((left, right) => {
      ...
      if (sortBy === "score") {
        return (scores.get(right.oracle_id)?.overall_score ?? -1)
    ...
    <section className="printing-picker" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="printing-picker-title" aria-describedby="printing-picker-description" aria-busy={printings.length === 0 && error === null} tabIndex={-1} onClick={(event) => { event.stopPropagation(); }}>
      ...
      <div className="printing-grid">
        {printings.map((printing) => {
          ...
          return <article className={`printing-option${selected ? " selected" : ""}`} key={printing.id}>
            <ClickableCardImage card={printing} className="printing-image" />
            ...
            <button disabled={selected} onClick={() => { select(printing, finish); }}>{selected ? "Current printing" : "Use this printing"}</button>
          </article>;
        })}
      </div>
    </section>
...
function ImageCard({
  ...
  >
    ...
    {!stacked && card.quantity > 1 && <span className="card-quantity" aria-label={`${String(card.quantity)} copies`}>×{card.quantity}</span>}
    {score !== null && <span className="card-score" aria-label={`${String(score.overall_score)} role score`} title={score.roles.map((role) => `${titleize(role.role)} ${String(role.score)}`).join(", ")}>{score.overall_score}</span>}
...
function VisualCardGroups({
  ...
  return <div className={`visual-groups ${view}`}>
    {groupedCards(cards, groupBy, sortBy, provider, scores).map((group) => <section className="visual-group" key={group.label}>
      <h3>{group.label} <small>{group.quantity}</small></h3>
      <div className={view === "stacks" ? "card-stacks" : "visual-card-grid"}>
        {group.cards.flatMap((card) => Array.from({ length: view === "stacks" ? card.quantity : 1 }, (_, index) => (
          <ImageCard
            card={card}
            add={() => { addCard(card); }}
            disabled={busy}
...
function DeckInfoView({
  ...
  return <section className="info-view" aria-labelledby="deck-info-title">
    <div className="view-heading">
      <div><span className="eyebrow">Deck information</span><h2 id="deck-info-title">Purpose and overview</h2></div>
      <button className="secondary-button" onClick={edit}><MaterialIcon name="edit" /> Edit deck info</button>
    </div>
    <div className="info-grid">
      <article className="info-card prominent">
        <span className="eyebrow">Goal / North Star</span>
        {deck.goal === "" ? <p className="muted">Define what this deck should consistently accomplish.</p> : <GeneratedDescription description={deck.goal} cards={deck.cardsets} />}
      </article>
      <article className="info-card">
        <span className="eyebrow">About this deck</span>
        {deck.description === "" ? <p className="muted">No user description yet.</p> : <GeneratedDescription description={deck.description} cards={deck.cardsets} />}
      </article>
      <article className="info-card ai-overview">
        <div className="info-card-heading"><span className="eyebrow">AI-generated overview</span><button className="text-button" disabled={busy} onClick={refreshOverview}><MaterialIcon name="refresh" /> Refresh overview</button></div>
        {deck.generated_description === ""
          ? <p className="muted" role="status">{busy ? "Generating an overview…" : "An overview will be generated when this view opens."}</p>
          : <GeneratedDescription description={deck.generated_description} cards={deck.cardsets} />}
      </article>
      <article className="info-card details-card">
        <span className="eyebrow">Deck details</span>
        <dl>
          <div><dt>Format</dt><dd>{titleize(deck.format)}</dd></div>
          <div><dt>Cards</dt><dd>{validation?.card_count ?? deck.cardsets.reduce((total, card) => total + card.quantity, 0)}</dd></div>
          <div><dt>Status</dt><dd>{validation?.valid === true ? "Valid" : "Needs attention"}</dd></div>
          <div><dt>Version</dt><dd>{deck.revision}</dd></div>
        </dl>
      </article>
    </div>
  </section>;
...
function DeckScoresView({
  ...
  const [evaluationSort, setEvaluationSort] = useState<"overall" | CardRole>("overall");
  const uniqueCards = new Set(deck.cardsets.map((card) => card.oracle_id)).size;
  const cardNames = new Map(deck.cardsets.map((card) => [card.oracle_id, card.card_name]));
  const cardsByOracleId = new Map(deck.cardsets.map((card) => [card.oracle_id, card]));
  const roleScore = (evaluation: CardRoleEvaluation, role: CardRole): number =>
    evaluation.roles.find((item) => item.role === role)?.score ?? -1;
  const rankedScores = [...scores.values()].sort((left, right) =>
  ...
  return <section className="scores-view" aria-labelledby="scores-title">
    <div className="view-heading">
      <div>
        <span className="eyebrow">Card evaluations</span>
        <h2 id="scores-title">Scores and role fit</h2>
      </div>
      <button disabled={scoring || uniqueCards === 0 || deck.goal.trim() === ""} onClick={scoreCards}>
        <MaterialIcon name="fact_check" /> {scoring ? "Evaluating cards…" : "Evaluate cards"}
      </button>
    </div>
    {deck.goal.trim() === "" && <section className="evaluation-goal-required" role="status">
      <div><MaterialIcon name="flag" /><span><strong>Add a Goal / North Star before evaluating cards</strong><small>Scores judge how well each card supports the deck's intended game plan.</small></span></div>
      <button className="secondary-button" onClick={editGoal}><MaterialIcon name="edit" /> Add goal</button>
    </section>}
    <div className="evaluation-summary">
      <div className="evaluation-count">
        <strong>{progress === null ? scores.size : progress.completed}/{uniqueCards}</strong>
        <span>cards evaluated</span>
      </div>
      {progress === null
        ? <p className="muted">Cards are tagged by role, judged against role-specific qualitative rubrics, and scored deterministically from those answers.</p>
        : <p className="muted" role="status">Evaluating cards at {progress.average_seconds_per_card === null ? "an estimated rate" : `${progress.average_seconds_per_card.toFixed(1)} seconds per card`}. {progress.eta_seconds === null ? "Estimating time remaining…" : `About ${formatDuration(progress.eta_seconds)} remaining.`}</p>}
    </div>
    {rankedScores.length > 0 && <div className="score-breakdown">
      <label className="compact-control evaluation-sort">Sort evaluations<select aria-label="Evaluation sort" value={evaluationSort} onChange={(event) => { setEvaluationSort(event.target.value as "overall" | CardRole); }}>
        <option value="overall">Overall score</option>
        {(["land", "mana_ramp", "card_advantage", "removal", "board_wipe", "enabler", "enhancer", "payoff"] as const).map((role) => <option key={role} value={role}>{titleize(role)}</option>)}
      </select></label>
      <div className="score-breakdown-list">
        {rankedScores.map((score) => {
          const card = cardsByOracleId.get(score.oracle_id);
          return <article className="score-card-row" key={score.oracle_id}>
          {card !== undefined && <ClickableCardImage card={card} className="score-card-image" />}
          <details className="score-card">
          <summary><span><strong>{cardNames.get(score.oracle_id) ?? score.oracle_id}</strong><small>{score.roles.map((role) => titleize(role.role)).join(" · ")}</small></span><b>{score.overall_score}</b></summary>
          <p><InlineCardText text={score.overall_comment} /></p>
          <div className="role-score-list">{score.roles.map((role) => <section className="role-score" key={role.role}>
            <header><b className={`role-tag ${role.role}`}>{titleize(role.role)}</b><strong>{role.score}</strong></header>
            <p><InlineCardText text={role.description} /></p>
            <ul>{role.answers.map((answer) => <li key={answer.criterion_id}><strong>{titleize(answer.criterion_id)}</strong><b>{titleize(answer.rating)} · {answer.score}</b></li>)}</ul>
          </section>)}</div>
          </details>
        </article>;
        })}
  ...
  return (
    <main aria-busy={busy}>
      ...
      <section className="page-heading">
        <div><h1>{mode === "decks" ? "Your decks" : "Import a deck"}</h1><p>{mode === "decks" ? "Build and manage your deck collection." : "Resolve a decklist and review every selected printing."}</p></div>
        {mode === "decks" && <button ref={addDeckButtonRef} onClick={() => { setShowAddDeck(true); }}>Add Deck</button>}
      </section>
      {error !== null && <p className="notice error" role="alert">{error}</p>}
      {mode === "import" && <section className="import-panel">
        ...
        <div className="import-preview">
          ...
          <div className="import-preview-scroll">
            ...
            {preview?.used_ai_fallback === true && <p className="notice" role="status">AI-assisted import extracted cards from the supplied text. Review the resolved cards before creating the deck.</p>}
            {preview !== null && <div className="preview-grid">
              {preview.cardsets.map((card) => (
                <article className="preview-card" key={`${card.printing_id}-${card.finish}-${card.zone}`}>
                  <ClickableCardImage card={card.scryfall} className="preview-image" />
          ...
          <footer className="import-actions">
            {preview === null
              ? <p className="muted">Preview the decklist before creating the deck.</p>
              : <button disabled={busy || preview.errors.length > 0 || title.trim() === ""} onClick={() => void createImportedDeck()}>Create imported deck</button>}
          </footer>
        </div>
      </section>}
      {mode === "decks" && <section className="deck-grid">
        {decks.map((deck) => (
          <article className="deck-card" key={deck.id}>
            ...
            <div className="deck-cover">
              {deck.cardsets.slice(0, 3).map((card) => (
                <ClickableCardImage key={card.id} card={card} className="cover-image" />
              ))}
            </div>
...
function Editor() {
  ...
  const [scores, setScores] = useState<Map<string, CardRoleEvaluation>>(new Map());
  const [scoring, setScoring] = useState(false);
  const [evaluationProgress, setEvaluationProgress] = useState<CardEvaluationProgress | null>(null);
  const [busy, setBusy] = useState(false);
  ...
  const [advisorWidth, setAdvisorWidth] = useState(() => {
    const stored = Number.parseInt(localStorage.getItem("survail.advisor-width") ?? "", 10);
  ...
  const [guidanceDecisions, setGuidanceDecisions] = useState<Record<string, "approved" | "rejected">>({});
  ...
  const evaluateCurrentDeck = useCallback(async (): Promise<void> => {
    ...
    setScoring(true);
    try {
      setEvaluationProgress({ completed: 0, total: new Set(deck.cardsets.map((card) => card.oracle_id)).size, average_seconds_per_card: null, eta_seconds: null });
      const loadedScores = await api.streamCurrentDeckEvaluation(
        deck.id,
        setEvaluationProgress,
        (result) => { setScores((current) => new Map(current).set(result.oracle_id, result)); },
      );
      setScores(new Map(loadedScores.map((score) => [score.oracle_id, score])));
      setAnnouncement(`${String(loadedScores.length)} cards scored`);
    } catch (reason) {
      setError(reason instanceof Error ? messageFor(reason) : "Could not score cards");
    } finally {
      setScoring(false);
  ...
  useEffect(() => {
    if (editorView === "info" && deck?.generated_description === "") void handleGenerateDescription();
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      ...
      if (viewport !== null && message !== null) {
        viewport.scrollTo({ top: message.offsetTop - viewport.offsetTop, behavior: "smooth" });
      }
    });
  ...
  async function handleSearch(event: SyntheticEvent<HTMLFormElement>): Promise<void> {
    ...
    setError(null);
    try {
      const cards = (await api.search(queryForDeckFormat(query, deck.format), printingPreferences)).cards;
      setResults(cards);
  ...
  async function handleGenerateDescription(refresh = false): Promise<void> {
    ...
    try {
      ...
      setDeck((current) => current === null ? null : {
        ...current,
  ...
  function resizeAdvisorWithKeyboard(event: React.KeyboardEvent<HTMLDivElement>): void {
    if (event.key !== "ArrowLeft" && event.key !== "ArrowRight" && event.key !== "Home") return;
    event.preventDefault();
    if (event.key === "Home") setAdvisorWidth(400);
    else setAdvisorWidth((current) => Math.max(320, current + (event.key === "ArrowLeft" ? 24 : -24)));
  }
  ...
  async function applyBulkEdit(): Promise<void> {
    ...
    setError(null);
    try {
      const preview = await api.importMoxfield(bulkDecklist, printingPreferences, true);
      ...
      const result = await api.applyOperation(
        deck.id, deck.revision, changes, "Bulk edit decklist",
      );
      setDeck(result.deck);
      setValidation(result.validation);
      ...
      setError(reason instanceof Error ? messageFor(reason) : "Could not update decklist");
    } finally {
      setBusy(false);
    }
  }
  ...
  function receiveAgentEvent(event: AgentUiEvent): void {
    console.info("deck-agent event received", { deckId: id, runId: event.run_id, eventType: event.type });
    setAgentEvents((current) => [...current, event]);
    if (event.type === "operation_applied") void refreshDeckAfterAgentOperation();
  }
  ...
  async function decideGuidanceProposal(
    ...
    setError(null);
    try {
      await api.decideGuidanceProposal(id, proposalId, expectedRevision, decision);
      ...
      setError(reason instanceof Error ? messageFor(reason) : "Could not decide guidance proposal");
      if (reason instanceof ApiError && reason.status === 409) await loadDeck();
    } finally {
      setBusy(false);
    }
  ...
  async function refreshDeckAfterAgentOperation(): Promise<void> {
    try {
      ...
      setError(reason instanceof Error ? messageFor(reason) : "Could not refresh the updated deck");
    }
  }
  ...
  async function submitAgentMessage(message: string): Promise<void> {
    ...
    setAgentEvents((current) => [
      ...current,
      { type: "user_message", run_id: userMessageId, payload: { message: cleanedMessage } },
    ]);
    setLatestUserMessageId(userMessageId);
    setAgentBusy(true);
    console.info("deck-agent message started", { deckId: id, localMessageId: userMessageId });
    try {
      const outcome = await api.sendAgentMessage(id, await ensureConversation(), cleanedMessage, receiveAgentEvent);
      console.info("deck-agent message stream closed", { deckId: id, localMessageId: userMessageId, outcome });
    } catch (reason) {
      console.error("deck-agent message failed", { deckId: id, localMessageId: userMessageId, reason });
      setAgentEvents((current) => [...current, {
        type: "stream_closed",
        run_id: userMessageId,
        payload: { expected: false, message: "The deck advisor could not be reached. Please try again." },
      }]);
      setError(reason instanceof Error ? messageFor(reason) : "Could not contact deck advisor");
    } finally {
      setAgentBusy(false);
    }
  }
  ...
  function handleAgentComposerKeyDown(event: React.KeyboardEvent<HTMLTextAreaElement>): void {
  ...
  return (
    ...
    <main className={`editor ${showAgent ? "advisor-open" : ""}`} style={{ "--advisor-width": `${String(advisorWidth)}px` } as React.CSSProperties} aria-busy={busy}>
        ...
        <div className="deck-app-bar" aria-label="Deck controls">
          ...
          <details className="validation-menu">
            <summary className={validation?.valid === true ? "validation-summary valid" : "validation-summary invalid"} aria-label={`${validation?.valid === true ? "Valid deck" : "Deck needs attention"}, ${String(validation?.card_count ?? 0)} cards`}>
              {validation?.valid === true ? <MaterialIcon name="check" /> : <MaterialIcon name="error" />}
            </summary>
            <div className="subheader-menu">
              ...
              {groupedValidationErrors(validation).map((group) => <details className="validation-error-group" key={group.errorId}>
                ...
                <div>{group.errors.map((validationError, index) => <p key={`${validationError.error_id}-${String(index)}`}><InlineCardText text={validationError.message} /></p>)}</div>
              </details>)}
            </div>
          </details>
          <button className="secondary-button labeled-action" onClick={() => { setShowEditDeck(true); }}><MaterialIcon name="edit" /> Edit</button>
          <button className={`secondary-button labeled-action ${showAgent ? "selected" : ""}`} aria-pressed={showAgent} onClick={() => { setShowAgent((current) => !current); }}><MaterialIcon name="forum" /> Advisor</button>
          <details className="overflow-menu">
            <summary className="icon-action" aria-label="More deck actions"><MaterialIcon name="more_vert" /></summary>
            <div className="subheader-menu action-menu">
              <button onClick={openBulkEdit}><MaterialIcon name="edit_note" /> Bulk edit decklist</button>
              <button onClick={() => { setShowHistory(true); }}><MaterialIcon name="history" /> History</button>
              <button className="danger" onClick={() => void handleDelete()}><MaterialIcon name="delete" /> Delete deck</button>
            </div>
          </details>
        </div>
        <nav className="editor-tabs" aria-label="Deck views">
          {(["cards", "scores", "info"] as const).map((view) => <button className={editorView === view ? "active" : ""} aria-current={editorView === view ? "page" : undefined} key={view} onClick={() => { setEditorView(view); }}>{titleize(view)}</button>)}
        </nav>
        {editorView === "cards" && <div className="deck-toolbar" aria-label="Card display controls">
          ...
          <div className="view-selector" aria-label="Card view">
            {(["stacks", "grid", "text"] as const).map((view) => <button className={deckView === view ? "active" : ""} aria-pressed={deckView === view} key={view} onClick={() => { setDisplayPreferences((current) => ({ ...current, view })); }}>{titleize(view)}</button>)}
          </div>
          <details className="organize-menu">
            <summary className="secondary-button"><MaterialIcon name="tune" /> Organize</summary>
            <div className="subheader-menu">
              <label>Group by<select value={groupBy} onChange={(event) => { setDisplayPreferences((current) => ({ ...current, groupBy: event.target.value as GroupBy })); }}><option value="type">Type</option><option value="color">Color</option><option value="mana-value">Mana Value</option></select></label>
              <label>Sort by<select aria-label="Card sort" value={sortBy} onChange={(event) => { setDisplayPreferences((current) => ({ ...current, sortBy: event.target.value as SortBy })); }}><option value="alphabetical">Alphabetical</option><option value="mana-value">Mana Value</option><option value="price">Price</option><option value="score">Role Score</option></select></label>
            </div>
          </details>
        </div>}
        {editorView === "info" && <DeckInfoView deck={deck} edit={() => { setShowEditDeck(true); }} refreshOverview={() => void handleGenerateDescription(true)} busy={busy} validation={validation} />}
        {editorView === "scores" && <DeckScoresView deck={deck} scores={scores} scoring={scoring} scoreCards={() => void evaluateCurrentDeck()} progress={evaluationProgress} editGoal={() => { setShowEditDeck(true); }} />}
        {editorView === "cards" && zonesFor(deck.format).map((zone) => {
          const cards = deck.cardsets.filter((card) => card.zone === zone);
          const groups = groupedCards(cards, groupBy, sortBy, priceProvider, scores);
          ...
          return <section className="zone" key={zone}><h2>{zoneLabel(zone)} <small>{cards.reduce((total, card) => total + card.quantity, 0)}</small></h2>
            {deckView === "text"
              ...
              : <VisualCardGroups cards={cards} view={deckView} groupBy={groupBy} sortBy={sortBy} format={deck.format} openPrinting={(card) => { setPrintingCardset(card); }} addCard={(card) => { changeQuantity(card, 1); }} removeCard={(card) => { changeQuantity(card, -1); }} markCommander={markAsCommander} busy={busy} scores={scores} />}
          </section>;
        })}
      </section>
      ...
      {showAgent && <div className="advisor-resizer" role="separator" aria-label="Resize deck advisor" aria-orientation="vertical" aria-valuemin={320} aria-valuenow={advisorWidth} tabIndex={0} onDoubleClick={() => { setAdvisorWidth(400); }} onKeyDown={resizeAdvisorWithKeyboard} onPointerDown={beginAdvisorResize} onPointerMove={resizeAdvisor} />}
      {showAgent && <aside className="agent-drawer" aria-labelledby="agent-title">
        <div className="page-heading"><div><h2 id="agent-title">Deck advisor</h2><p>Ask questions or review proposed changes.</p></div><button className="icon-action" aria-label="Close deck advisor" onClick={() => { setShowAgent(false); }}><MaterialIcon name="close" /></button></div>
        <div className="agent-events" ref={agentEventsRef} aria-live="polite">
          {agentEvents.length === 0 && <div className="agent-starters">
            <p className="muted">Ask about strategy, weaknesses, card choices, or possible changes.</p>
            <div className="agent-starter-chips" aria-label="Suggested questions">
              {["What does this deck do?", "What is this deck missing?", "Which cards should I add or remove?"].map((prompt) => <button key={prompt} disabled={agentBusy} onClick={() => void submitAgentMessage(prompt)}>{prompt}</button>)}
            </div>
          </div>}
          {agentEvents.map((event, index) => {
            const key = `${event.run_id}-${String(index)}`;
            if (event.type === "user_message") return <article className="agent-user-message" ref={event.run_id === latestUserMessageId ? latestUserMessageRef : undefined} key={key}><small>You</small><InlineCardText text={event.payload.message} /></article>;
            if (isAgentActivityEvent(event)) {
              const superseded = agentEvents.slice(index + 1).some((later) => later.run_id === event.run_id && (isAgentActivityEvent(later) || later.type === "run_completed" || later.type === "run_failed" || later.type === "stream_closed"));
              return superseded || !agentBusy ? null : <p className="agent-status active" role="status" key={key}><span aria-hidden="true" />{event.payload.message}</p>;
            }
            if (event.type === "assistant_text_delta") {
              if (agentEvents.slice(0, index).some((prior) => prior.run_id === event.run_id && prior.type === "assistant_text_delta")) return null;
              if (agentEvents.slice(index + 1).some((later) => later.run_id === event.run_id && later.type === "assistant_completed")) return null;
                          const streamedText = streamedAgentText(agentEvents, event.run_id);
              return <article className="agent-message streaming" key={key}><GeneratedDescription description={visibleStreamingText(streamedText)} cards={deck.cardsets} /></article>;
            }
            if (event.type === "assistant_completed") return <article className="agent-message" key={key}><GeneratedDescription description={event.payload.message} cards={deck.cardsets} /></article>;
            if (event.type === "run_failed") return <p className="notice error" role="alert" key={key}>{event.payload.message}</p>;
            if (event.type === "card_results") return <p className="agent-status" key={key}>{event.payload.cards.length} matching cards found.</p>;
            if (event.type === "guidance_proposal") {
              const decision = guidanceDecisions[event.payload.proposal_id];
              return <article className="agent-guidance-proposal" key={key}>
                <span className="eyebrow">Your approval is required</span>
                <strong>Update deck guidance?</strong>
                <p><InlineCardText text={event.payload.reason} /></p>
                {event.payload.proposed_goal !== null && <blockquote><InlineCardText text={event.payload.proposed_goal} /></blockquote>}
                {decision === undefined
                  ? <div className="button-row"><button disabled={busy} onClick={() => void decideGuidanceProposal(event.payload.proposal_id, event.payload.expected_revision, "approve")}>Approve</button><button className="secondary-button" disabled={busy} onClick={() => void decideGuidanceProposal(event.payload.proposal_id, event.payload.expected_revision, "reject")}>Reject</button></div>
                  : <p className="agent-stream-state complete"><MaterialIcon name={decision === "approved" ? "check" : "close"} />Proposal {decision}</p>}
              </article>;
            }
            if (event.type === "operation_applied") return <p className={`notice ${event.payload.validation.valid ? "success" : "error"}`} key={key}>Deck change applied{event.payload.validation.errors.length > 0 ? ` with ${String(event.payload.validation.errors.length)} validation issues` : ""}.</p>;
            if (event.type === "stream_closed") {
              const superseded = agentEvents.slice(index + 1).some((candidate) => candidate.run_id === event.run_id);
              if (event.payload.expected && superseded) return null;
              return event.payload.expected
                ? <p className="agent-stream-state complete" key={key}><MaterialIcon name="check" />{event.payload.message}</p>
                : <p className="agent-stream-state interrupted" role="status" key={key}><MaterialIcon name="wifi_off" />{event.payload.message}</p>;
            }
            return null;
          })}
        </div>
        <form className="agent-composer" onSubmit={sendAgentMessage}>
          <label className="sr-only" htmlFor="agent-message">Message deck advisor</label>
          <textarea id="agent-message" rows={2} value={agentMessage} onChange={(event) => { setAgentMessage(event.target.value); }} onKeyDown={handleAgentComposerKeyDown} placeholder="Ask about this deck…" />
          <button className="icon-action" disabled={agentBusy || agentMessage.trim() === ""} aria-label="Send message"><MaterialIcon name="send" /></button>
        </form>
      </aside>}
      {showBulkEdit && <div className="modal-backdrop" onClick={() => { setShowBulkEdit(false); }}>
        <section className="bulk-edit-modal" ref={bulkEditDialogRef} role="dialog" aria-modal="true" aria-labelledby="bulk-edit-title" aria-describedby="bulk-edit-description" tabIndex={-1} onClick={(event) => { event.stopPropagation(); }}>
          <div className="page-heading"><div><h2 id="bulk-edit-title">Bulk edit decklist</h2><p className="muted" id="bulk-edit-description">Edit quantities, cards, or sections as free text. Changes are applied together.</p></div><button className="icon-action" aria-label="Close bulk decklist editor" onClick={() => { setShowBulkEdit(false); }}><MaterialIcon name="close" /></button></div>
          {bulkEditErrors.length > 0 && <div className="notice error" role="alert">{bulkEditErrors.map((message) => <p key={message}>{message}</p>)}</div>}
          <label className="bulk-edit-field">Decklist<textarea aria-label="Decklist" value={bulkDecklist} onChange={(event) => { setBulkDecklist(event.target.value); }} spellCheck={false} /></label>
          <div className="button-row bulk-edit-actions"><button className="secondary-button" disabled={busy} onClick={() => { setShowBulkEdit(false); }}>Cancel</button><button disabled={busy || bulkDecklist.trim() === ""} onClick={() => void applyBulkEdit()}>{busy ? "Applying changes…" : "Apply changes"}</button></div>
        </section>
      </div>}
      {showEditDeck && <div className="modal-backdrop" onClick={() => { setShowEditDeck(false); }}>
        <form className="add-deck-modal guidance-edit-modal stack" ref={editDeckDialogRef} role="dialog" aria-modal="true" aria-labelledby="edit-deck-title" aria-describedby="edit-deck-description" tabIndex={-1} onSubmit={(event) => { event.preventDefault(); void handleSaveDetails(event).then((saved) => { if (saved) setShowEditDeck(false); }); }} onClick={(event) => { event.stopPropagation(); }}>
          ...
          <p className="muted" id="edit-deck-description">Define the deck's North Star. Card roles and scores are evaluated automatically.</p>
          <label>Title<input autoFocus maxLength={120} required value={title} onChange={(event) => { setTitle(event.target.value); }} /></label>
          <label>Description<textarea value={description} onChange={(event) => { setDescription(event.target.value); }} /></label>
          <label>Goal / North Star<textarea value={goal} onChange={(event) => { setGoal(event.target.value); }} placeholder="What should this deck consistently accomplish?" /></label>
      ...
      {showHistory && <div className="modal-backdrop" onClick={() => { setShowHistory(false); }}>
        <section className="history-modal" ref={historyDialogRef} role="dialog" aria-modal="true" aria-labelledby="history-title" aria-describedby="history-description" tabIndex={-1} onClick={(event) => { event.stopPropagation(); }}>
          <div className="page-heading"><div><h2 id="history-title">Deck history</h2><p id="history-description">{operations.length} recorded {operations.length === 1 ? "change" : "changes"}</p></div><button className="icon-action" aria-label="Close deck history" onClick={() => { setShowHistory(false); }}><MaterialIcon name="close" /></button></div>
          <div className="history-list">
            {operations.length === 0 && <p className="muted">No changes have been recorded.</p>}
            {operations.map((operation) => <details className="history-entry" key={operation.id}>
              <summary><span><strong><InlineCardText text={operation.reason ?? "Deck update"} /></strong><small>Version {operation.revision_after} · {new Date(operation.created_at).toLocaleString()} · {operation.changes.length} {operation.changes.length === 1 ? "change" : "changes"}</small></span><button disabled={busy} onClick={(event) => { event.preventDefault(); void handleRevert(operation); }}>Undo</button></summary>
              <div>{operation.changes.map((change, index) => <small key={`${change.printing_id}-${String(index)}`}>{change.quantity_delta > 0 ? "+" : ""}{change.quantity_delta} <InlineCardText text={`[[${change.card_name}]]`} /> · {zoneLabel(change.zone)}</small>)}</div>
            </details>)}
          </div>
        </section>
      </div>}
