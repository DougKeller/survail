...
const CardPresentationContext = createContext<CardPresentationContextValue | null>(null);
...
function displayPrice(label: string, value: string | null | undefined): ReactNode {
    ...
    <div className="card-presentation-backdrop" onMouseDown={close}>
      ...
      >
        <header className="card-details-header">
          ...
          <button aria-label="Close card details" className="card-details-close" onClick={close} type="button">
        ...
        <div className="card-details-content">
          <div className="card-details-art">
            {image === null
              ? <div aria-label={`${card.name}, image unavailable`} className="card-presentation-placeholder" role="img">{card.name}</div>
              : <img alt={card.name} src={image} />}
          </div>
          <div className="card-details-copy">
            {card.mana_cost !== null && <p className="card-details-mana"><strong>Mana cost</strong><span>{card.mana_cost}</span></p>}
            <p className="card-details-oracle">{card.oracle_text ?? "Oracle text unavailable."}</p>
            <dl className="card-details-facts">
              <span><dt>Set</dt><dd>{card.set_name} ({card.set.toUpperCase()})</dd></span>
              <span><dt>Rarity</dt><dd>{card.rarity}</dd></span>
              {details.finish !== null && <span><dt>Finish</dt><dd>{details.finish}</dd></span>}
              {details.finish === null && card.finishes.length > 0 && <span><dt>Finishes</dt><dd>{card.finishes.join(", ")}</dd></span>}
              {card.released_at !== undefined && card.released_at !== null && <span><dt>Released</dt><dd>{card.released_at}</dd></span>}
            </dl>
            {card.prices !== undefined && <section aria-labelledby={`${titleId}-prices`} className="card-details-prices">
...
export function CardPresentationProvider({
  ...
  const [selectedCard, setSelectedCard] = useState<CardPresentationSource | null>(null);
  const cardsByName = useMemo(() => new Map(
    cards.map((card) => [cardName(card).toLocaleLowerCase(), card] as const),
  ), [cards]);
  ...
  const value = useMemo(() => ({ cardsByName, openCard }), [cardsByName, openCard]);

  return <CardPresentationContext.Provider value={value}>
    {children}
    {selectedCard !== null && <CardDetailsModal close={closeCard} source={selectedCard} />}
...
export function useCardPresentation(): CardPresentationContextValue {
  ...
  if (context === null) {
    throw new Error("useCardPresentation must be used within CardPresentationProvider");
  }
  return context;
}
...
export function ClickableCardImage({
  ...
  >
    {source === null
      ? <span className="card-presentation-placeholder"><span aria-hidden="true" className="material-symbols-outlined">image_not_supported</span>{name}</span>
      : <img alt={name} loading={loading} src={source} />}
  </button>;
}

function InlineCardReference({ card, label }: { card: CardPresentationSource; label: string }) {
  const { openCard } = useCardPresentation();
  ...
  const updatePreviewPosition = useCallback((): void => {
    ...
    const left = preferredLeft + previewWidth <= window.innerWidth - 16
      ? preferredLeft
  ...
  return <span className="inline-card-reference">
    <button
    ...
    {previewVisible && source !== null && previewStyle !== null && createPortal(
      <span
        className="inline-card-preview"
        role="tooltip"
        style={{ left: `${String(previewStyle.left)}px`, top: `${String(previewStyle.top)}px` }}
      >
        <img alt="" src={source} />
      </span>,
      document.body,
...
export function InlineCardText({
  ...
  const localCards = useMemo(() => new Map(
    cards.map((card) => [cardName(card).toLocaleLowerCase(), card] as const),
  ), [cards]);
  const pattern = /\[\[([^[\]]+)\]\]/g;
  ...
  for (const match of text.matchAll(pattern)) {
    ...
    if (index > cursor) content.push(<span key={String(key++)}>{text.slice(cursor, index)}</span>);
    const card = localCards.get(matchedName.toLocaleLowerCase())
      ?? context.cardsByName.get(matchedName.toLocaleLowerCase());
    content.push(card === undefined
      ? <span key={String(key++)}>{matchedName}</span>
      : <InlineCardReference card={card} key={String(key++)} label={cardName(card)} />);
    cursor = index + fullMatch.length;
  }
  if (cursor < text.length) content.push(<span key={String(key)}>{text.slice(cursor)}</span>);
