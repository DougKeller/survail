...
from survail.deck_agent.events import AgentEventSink, AgentUiEvent
from survail.domain.deck_operations import DeckOperationError, apply_deck_operation
from survail.domain.decks import deck_validation_summary, validate_deck
...
from survail.domain.role_evaluation import OpenAIRoleEvaluator, evaluate_oracle_ids
...
from survail.telemetry import observe_agent_run, record_agent_model_phase, record_agent_tool_call
...
async def search_legal_cards(context: RunContextWrapper[DeckAgentContext], query: str) -> str:
    ...
    payload = {
        "status": "completed",
        "message": "Legal card search completed; no deck changes were made.",
        "current_revision": deck.revision,
        "query": query,
...
async def _execute_observed(
    ...
    try:
        ...
        result = Runner.run_streamed(build_agent(), prompt, context=context)
